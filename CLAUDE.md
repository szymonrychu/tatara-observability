...
The platform's observability-as-code: agent-adjustable Grafana alert rules,
applied to Grafana via terraform on merge. Agents edit only `alerts/tatara-*.yaml`
- the simple `name` / `queries[].expression` / `math_operator` / `threshold` /
`for` / `keep_firing_for` / `decimal_points` / `annotations` / `labels` schema.
...
too; `scripts/check_alert_schema.py` fails CI on any undeclared key. A PR posts
a sticky `terraform plan`; merge to `main` runs `terraform apply` (Grafana
Editor SA token, S3 state).

Alerts carry `homelab=true` + `system=tatara` + `component` + `severity`. A
firing tatara alert routes to `/operator/webhooks/tatara/grafana` -> an
`incident` Task -> a brainstorming issue.

## What this repo is NOT

- Not a code component. No Go, no Dockerfile, no chart. Terraform and alert YAML.
- Not the routing. Contact points and the notification policy stay in
  `infra/terraform/grafana`; this repo owns the `Tatara` folder and the
  `tatara-*` rule groups. Routing is by label and works regardless of owner.
- Not dashboards (yet). Possible later; out of scope now.
...
- The silent-drop failure mode is the whole reason `check_alert_schema.py`
  exists. An alert attribute that is not in `variables.tf` does not error, it
  simply never reaches Grafana. Never bypass that check to land an alert.
- An alert that fires opens an incident Task and burns an agent pod. Tune
  thresholds against real series before merging, not after the first page.
