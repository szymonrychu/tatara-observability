...
| `keep_firing_for` | no | Grace period the rule keeps Firing after its condition clears (e.g. `30m`). Default `"0"` = not set (the provider rejects an empty string as an invalid duration). Use it to de-flap a rule whose fault resolves and re-fires, so one fault is one incident |
